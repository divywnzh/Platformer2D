...
public class GameObject {
	...
	protected void updateAnimationTick() {
		...
		if (aniTick >= ANI_SPEED) {
			aniTick = 0;
			aniIndex++;
			if (aniIndex >= GetSpriteAmount(objType)) {//One complete animation 
				aniIndex = 0;
				if(objType==BARREL || objType==BOX) {//Containers
					doAnimation=false;
					active=false;
				}else if(objType==CANNON_LEFT || objType==CANNON_RIGHT)//Cannon
					doAnimation=false;
			}		
		}
	}
